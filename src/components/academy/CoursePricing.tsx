...
export default function CoursePricing({ slug, title, durations }: Props) {
  const [selected, setSelected] = useState(durations[0]);

  const deposit = useMemo(() => Math.round(selected.price * 0.7), [selected]);
  const balance = useMemo(() => selected.price - deposit, [selected, deposit]);
    ...
    <section className="space-y-10">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {durations.map((duration) => (
          ...
          >
            <h3 className="text-2xl font-bold">{duration.months} Months</h3>
      ...
      <div className="rounded-3xl border bg-gray-50 p-8 shadow-sm">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            Pay only <strong>70%</strong> to secure your admission. The
            remaining balance can be paid before the end of your training.
          </p>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <h2 className="mt-2 text-3xl font-bold">₦{selected.price.toLocaleString()}</h2>
          </div>
        ...
        <div className="mt-8 rounded-2xl border border-blue-100 bg-blue-50 p-6">
          ...
          <p className="mt-1 text-gray-700">
            Duration: <strong>{selected.months} Months</strong>
          </p>
        </div>
